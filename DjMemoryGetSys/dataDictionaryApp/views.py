...
def index(request):
    if request.method == "GET":
        ...
        queryset = DataDict.objects.all().values()
        table = DataDictTable(queryset)
        RequestConfig(request, paginate={'per_page': settings.PAGE_NUM}).configure(table)
        return render(request, "dataDictionary.html",
                      context={"table": table, "Form": querytoolbar_form,
                               "formUrl": "dataDictionaryApp:CodetypeAll",
...
def CodetypeUpdate(request, pk):
    ...
    if request.method == 'GET':
        ...
        return render(request, "dataDictUpdate.html",
                      context={"dataDictUpdate_form": dataDictUpdate_form, "pk": pk, "title": "修改数据字典"})
        ...
        if dataDictUpdate_form.is_valid():
            try:
                ...
                return render(request, "dataDictUpdate.html",
                              context={"dataDictUpdate_form": dataDictUpdate_form, "msg": str(e), "title": "修改数据字典"})
        else:
            return render(request, "dataDictUpdate.html",
                          context={"dataDictUpdate_form": dataDictUpdate_form, "title": "修改数据字典"})


@xframe_options_sameorigin
@login_required()
def addCodetype(request):
    # 放假渲染的html, 前端接受后直接弹框显示，避免新增。修改时候将页面覆盖
    if request.method == 'GET':
        dataDictUpdate_form = DataDictUpdateForm()
        return render(request, "dataDictUpdate.html",
                      context={"dataDictUpdate_form": dataDictUpdate_form, "title": "新增数据字典"})
    elif request.method == "POST":
        ...
        if dataDictUpdate_form.is_valid():
            try:
                ...
                print("addCodetype", e)
                return render(request, "dataDictUpdate.html",
                              context={"dataDictUpdate_form": dataDictUpdate_form, "msg": str(e), "title": "新增数据字典"})
        else:
            return render(request, "dataDictUpdate.html",
                          context={"dataDictUpdate_form": dataDictUpdate_form, "title": "新增数据字典"})
...
def CodetypeAll(request):
    ...
    return render(request, "dataDictionary.html",
                  context={"table": table, "Form": querytoolbar_form,
                           "formUrl": "dataDictionaryApp:CodetypeAll", "formAddUrl": "dataDictionaryApp:addCodetype"})
