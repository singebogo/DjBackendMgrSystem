$(function () {
    ...
    $('#wechatImport').change(function (e) {
        ...
        readFile(e, '微信昵称', '微信支付账单明细列表', "");
    });

    $('#ailplayImport').change(function (e) {
        $('#fileImportTable').bootstrapTable("destroy");
        readFile(e, '#账号', '账务明细列表', "账务明细列表结束");
    });


    function readFile(e, nickNameLabel, billLabel, billEndLabel) {
        if(!e.target.files[0]){
        ...
        reader.onloadend = function (e) {
            readerLoadend(e.target.result, fileType, rABS, nickNameLabel, billLabel, billEndLabel);
        }
    }

    function readerLoadend(data, fileType, rABS, nickNameLabel, billLabel, billEndLabel) {
        if (fileType == 'txt' || fileType == 'csv') {       //txt或csv文件直接读取
            ...
            dataImportFormat(content, nickNameLabel, billLabel, billEndLabel);
        }
    }

    function dataImportFormat(content, nickNameLabel, billLabel, billEndLabel) {
        if (content.length == 0) {
            return;
        }
        ...
        initFileImportTable(columns, billDatas);
    }
